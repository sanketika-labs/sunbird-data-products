...
case class CollectionBatchStatusMetrics(unStarted: Long, inProgress: Long, completed: Long)

object CollectionBatchStatusUpdaterJob extends IJob with BaseReportsJob {
  implicit val className: String = "org.sunbird.analytics.audit.CollectionBatchStatusUpdaterJob"
  val cassandraFormat = "org.apache.spark.sql.cassandra"
  private val collectionBatchDBSettings = Map("table" -> "batches", "keyspace" -> "sb_collection_tracking", "cluster" -> "LMSCluster")

  // $COVERAGE-OFF$ Disabling scoverage for main and execute method
  override def main(config: String)(implicit sc: Option[SparkContext], fc: Option[FrameworkContext]): Unit = {
    val jobName: String = "CollectionBatchStatusUpdaterJob"
  ...
  def execute(fetchData: (SparkSession, Map[String, String], String, StructType) => DataFrame)(implicit spark: SparkSession, fc: FrameworkContext, config: JobConfig, sc: SparkContext): CollectionBatchStatusMetrics = {
    ...
    JobLogger.log(s"Collection Batch status metrics before updating the table", Option(metrics), INFO)
    val res = updateBatchStatus(config, collectionBatchDF)
    collectionBatchDF.unpersist()
    res
  }

  def updateBatchStatus(updaterConfig: JobConfig, collectionBatchDF: DataFrame)(implicit sc: SparkContext, spark: SparkSession): CollectionBatchStatusMetrics = {
    ...
    CollectionBatchStatusMetrics(finalDF.filter(col("status") === 0).count(), finalDF.filter(col("status") === 1).count(), finalDF.filter(col("status") === 2).count())
  }
